...
function UserInquiries() {
  ...
  const { data, loading, error } = useFetch(process.env.REACT_APP_API_URL + "/inquiries" + withParams({
    limit: DEFAULT_LIMIT,
    ...
    <>
      ...
      <table className="inquiries-table">
        ...
        </tr>
        {
          ...
          loading && (
            <tr className="row-flex">
              <td className="col" colSpan="6">로딩중...</td>
            </tr>)
        }
        {
          // 에러 발생 시 "에러 발생" 출력
          error && (
            <tr className="row-flex">
              <td className="col" colSpan="6">에러 {error} 발생</td>
            </tr>
          )
        }
        ...
        {/* 내용 부분 */}
        {
          ...
          data?.inquiries?.length === 0 ?
            (<tr className="row-flex">
              <td className="col" colSpan="6">문의 내역이 없습니다.</td>
            </tr>)
            // 데이터가 있을 경우, 데이터를 출력
            : (

              data?.inquiries.map((inquiry) => (
                <tr className="row-flex" onClick={(e) => { navigiate("detail/" + inquiry.id) }}>
      ...
      <ol className="pagination">
        ...
        <span class={`ico material-symbols-outlined ${hasPreviousPage ? "" : "disable"}`}
          onClick={(e) => {
            setPage(page - 1)
          }}>
          keyboard_arrow_left
        ...
        {
          hasPreviousPage && (<>
            <li className="previous" onClick={(e) => setPage(page - 1)}>{+page - 1}</li>
          </>)
        }
        ...
        <span class={`ico material-symbols-outlined ${hasNextPage ? "" : "disable"}`}
